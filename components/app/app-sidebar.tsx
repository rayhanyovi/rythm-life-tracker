...
const moduleNavItems: AppNavItem[] = [
  {
    href: "/dashboard",
    label: "Tasks",
...
const taskNavGroups: AppNavGroup[] = [
  {
    label: "Views",
    items: [
      {
...
export function AppSidebar({
        ...
        <div className="rounded-xl border border-sidebar-border bg-background/80 p-4 shadow-xs">
          ...
          <div className="mt-3 grid grid-cols-2 gap-2">
            {moduleNavItems.map((item) => (
              <div key={item.label} className="flex items-center gap-3 rounded-xl border border-border bg-background px-3 py-3">
  ...
  return (
    <div className="flex h-full overflow-hidden rounded-[1.25rem] border border-sidebar-border bg-sidebar shadow-lg">
      <div className="flex w-14 shrink-0 flex-col items-center border-r border-sidebar-border bg-sidebar/95 px-2 py-3">
        <div className="flex size-10 items-center justify-center rounded-xl border border-sidebar-border bg-background text-sm font-semibold text-foreground shadow-xs">
          R
        </div>
        <div className="mt-5 flex flex-col gap-2">
          {moduleNavItems.map((item) => (
            <ModuleItem
              key={item.label}
              active={!item.disabled && item.label === "Tasks"}
              item={item}
              onNavigate={onNavigate}
            />
          ))}
        ...
        <div className="mt-auto flex flex-col items-center gap-3">
          <div className="flex size-10 items-center justify-center rounded-full border border-border bg-background/80 text-xs font-semibold text-foreground">
            {initials}
          </div>
        </div>
      </div>
        ...
        <div className="space-y-3 border-t border-sidebar-border px-2 pt-4">
          ...
          <div className="flex items-center justify-between gap-3 rounded-xl border border-border bg-background/80 px-3 py-3 shadow-xs">
            <div>
              ...
              <p className="mt-1 text-sm text-foreground">Personal only</p>
            </div>
            <SignOutButton />
          </div>
        </div>
