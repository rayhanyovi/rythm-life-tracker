...
You can skim 1 and 2 every time. Read 3 and 4 only when the task domain demands them.
...
| What's the IA? Tasks-first or Quest? | [DESIGN_DIRECTION.md](./DESIGN_DIRECTION.md) Section 2 + [PRODUCT_PLAN.md Open Strategic Decision 1](./PRODUCT_PLAN.md#1-ia-direction-quest-model-vs-tasks-first) |
...
- **Routes (canonical, do not rename):**
  - `/` — public landing or redirect to `/dashboard` if authenticated
  - `/sign-in`, `/sign-up`, `/forgot-password`, `/reset-password`
  - `/dashboard`, `/quests`, `/categories`, `/history`
  - `/offline`
- **Sidebar labels (current state, Tasks-first framing):**
  - `Today` → `/dashboard`
  - `Lists` → `/quests`
  - `Habit Lists` → `/categories`
  - `Activity Log` → `/history`
  - `Upcoming`, `Calendar`, `Journal` are **disabled placeholders** (visible but not clickable)
- **The Quest IA vs Tasks-first IA pivot is unsettled.** Until the user resolves it, every UI change must stay compatible with both labelings (route paths stay Quest, sidebar labels are Tasks-first). **Do not** rename routes, drop placeholders, or commit one-directional copy without explicit user instruction.
...
8. Preserve compatibility with both Quest IA and Tasks-first labels for sidebar / page-intro copy until the strategic decision lands.
...
2. Check if the feature depends on an Open Strategic Decision (IA direction, database provider, `Habit Lists` data model). If yes, surface the dependency to the user before implementing.
...
- ❌ Adding gamification (XP, levels, badges, fire-emoji streak symbols, celebrations).
...
- ❌ Silently picking the IA direction (Quest vs Tasks-first) without user input.
...
1. **Read** the relevant section of the relevant canonical doc(s). At minimum [PRODUCT_PLAN.md](./PRODUCT_PLAN.md) to confirm scope.
...
7. **Commit** as a separate commit per finished task with the format `<tag>: <task name>` (`feat`, `fix`, `refactor`, `docs`, `chore`, `ui`, `style`).
...
| IA, navigation, surface roles | [DESIGN_DIRECTION.md](./DESIGN_DIRECTION.md) Section 2 + [PRODUCT_PLAN.md](./PRODUCT_PLAN.md) Open Strategic Decision 1 |
...
If you find yourself reaching for animations, gradients, gamification, or analytics, you've drifted. Stop, re-read [PROJECT_OVERVIEW.md](./PROJECT_OVERVIEW.md), and pick a path that makes the product quieter, not louder.
